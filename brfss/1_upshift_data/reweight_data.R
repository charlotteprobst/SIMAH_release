...
data$new_weight <- round(data$final_sample_weight*0.1,digits=0)
...
for(i in names(dataFiles)){
  sampled[[paste(i)]] <- sample_function(dataFiles[[paste(i)]], unique(dataFiles[[paste(i)]]$originalsample))
}

sampled <- do.call(rbind, sampled)

USA <- sampled %>% mutate(State=="USA")
sampled <- rbind(USA, sampled)

# save RDS of the reweighted and reduced sample data 
saveRDS(sampled, "SIMAH_workplace/brfss/processed_data/reweighted_sampled_BRFSS.RDS")
