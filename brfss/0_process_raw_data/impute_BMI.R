...
# checking BMI over time 

summary <- data_new %>% 
  mutate(obese=ifelse(BMI>=30, 1,0),
         weightedobese=obese*final_sample_weight) %>% 
  group_by(YEAR, sex_recode) %>% 
  mutate(sumpop = sum(final_sample_weight)) %>% 
  summarise(raw_obesity_prevalence = mean(obese),
            weighted_obesity_prevalence = sum(weightedobese)/sumpop,
            meanBMI = mean(BMI)) %>% distinct()

ggplot(data=summary, aes(x=YEAR, y=raw_obesity_prevalence, colour=sex_recode)) + geom_line()

ggplot(data=summary, aes(x=YEAR, y=weighted_obesity_prevalence, colour=sex_recode)) + geom_line()

ggplot(data=summary, aes(x=YEAR, y=meanBMI, colour=sex_recode)) + geom_line()
