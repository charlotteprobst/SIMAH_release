    ...
    mutate(hispanic_comb = ifelse(YEAR<=2000, HISPANIC,
      ...
      race_comb = ifelse(YEAR<2001, ORACE,
                              ifelse(YEAR>=2001 & YEAR<=2012, RACE2,
                                     ifelse(YEAR>=2013 & YEAR<2022, X_RACE, 
                         ...
                         ifelse(YEAR==2001, ALCDAYS,
                                ifelse(YEAR>=2002 & YEAR<=2004, ALCDAY3,
                                       ifelse(YEAR>=2005 & YEAR<=2010, ALCDAY4, 
                                              ifelse(YEAR>=2011 & YEAR<2022, ALCDAY5, 
                                                     ifelse(YEAR==2022, ALCDAY4, NA)))))))
...
recode_interview <- function(data){
  if(data$YEAR[1]>=2020){
    data <- data %>% 
      mutate(
        interview_date = mdy(IDATE))
  }else{
    data <- data %>% 
      mutate(
        interview_date = mdy(IDATE))
...
recode_health_insurance <- function(data){
    if(data$YEAR[1]>=2021){
      data <- data %>% 
        mutate(insurance_detailed = ifelse(PRIMINSR == 1, "employer",
                                        ifelse(PRIMINSR==2, "private",
                                               ifelse(PRIMINSR==3, "medicare",
                                                      ifelse(employment_var==4, "medigap",
                                                             ifelse(employment_var==5, "medicaid",
                                                                    ifelse(employment_var==6, "CHIP",
                                                                           ifelse(employment_var==7, "military",
                                                                                  ifelse(employment_var==8, "indian",
                                                                                         ifelse(employment_var==9, "state",
                                                                                                ifelse(employment_var==10, "government_other",
                                                                                                       ifelse(employment_var==88, "none",
                                                                                                              ifelse(employment_var==77, NA,
                                                                                                              ifelse(employment_var==99, NA, NA))))))))))))),
           insurance = ifelse(insurance_detailed %in% c("employer", "private", "medicare",
                                                        "medigap", "medicaid", "CHIP", 
                                                        "military", "indian", "state", "government_other"), "insured",
                              ifelse(insurance_detailed == "none", "not insured", NA)))
    }else{
      data <- data %>% 
        mutate(insurance_detailed = ifelse(HLTHCVR1 == 1, "employer",
                                           ifelse(HLTHCVR1==2, "private",
                                                  ifelse(HLTHCVR1==3, "medicare",
                                                         ifelse(HLTHCVR1==4, "medicaid or other state",
                                                                ifelse(HLTHCVR1==5, "military",
                                                                       ifelse(HLTHCVR1==6, "indian",
                                                                              ifelse(HLTHCVR1==7, "other",
                                                                                     ifelse(HLTHCVR1==8, "none",
                                                                                            ifelse(employment_var==77, NA,
                                                                                                                 ifelse(employment_var==99, NA, NA)))))))))),
               insurance = ifelse(HLTHPLN1 == 1, "insured",
                                  ifelse(HLTHPLN1 == 2, "not insured", NA)))
    }
  return(data) 
  }
...
recode_healthcare_access <- function(data){
  data <- data %>% 
    mutate(
      MEDCOST = ifelse(YEAR==2021, MEDCOST1, MEDCOST),
      no_healthcare_access = ifelse(MEDCOST==1, 1,
                                ifelse(MEDCOST==2, 0,
                                       ifelse(MEDCOST==77, NA,
                                              ifelse(MEDCOST==99, NA, NA)))))
  return(data)
}
...
subset_data <- function(data){
  data <- data %>% 
    dplyr::select(YEAR, State, interview_date, 
                  strata_num = X_STSTR, psu_num = X_PSU, final_sample_weight, 
                  race_eth, race_eth_detailed, sex_recode, age_var,
                  education_summary, employment, marital_status,
                  household_income,
                  height_cm, weight_kg, BMI, BMI_final, BMI_derived, drinkingstatus, 
                  mentalhealth, physicalhealth,
                  # insurance, insurance_detailed, no_healthcare_access, 
