...
#
gunzip("SIMAH_workplace/ACS/usa_00036.dat.gz", remove=FALSE)

# now read in the data for 2021 
ddi <- read_ipums_ddi("SIMAH_workplace/ACS/usa_00036.xml")
...
summary <- data %>% 
  ...
  mutate(age_gp = cut(AGE, breaks=c(0,24,29,34,39,44,49,54,59,64,69,74,79,10000),
                      ...
                      sex = SEX) %>% 
  group_by(YEAR, state, sex, race, AGE, edclass) %>%
  summarise(TPop=sum(PERWT)) %>% rename(year=YEAR, ind_age=AGE)

summaryUSA <- summary %>% group_by(year, sex, race, ind_age, edclass) %>% 
  summarise(TPop = sum(TPop)) %>% mutate(state="USA")

summary <- rbind(summary, summaryUSA)

# now interpolate for the years in between 1980 and 1990 and 1990 and 2000 
# read in the other dataset for the 2000 data and join together 
ACS_2000 <- read.csv("SIMAH_workplace/ACS/ACS_popcounts_2000_2021_bystate_indage.csv")

wholepop <- rbind(summary, ACS_2000) %>% 
  filter(ind_age<=80 & ind_age>=18) %>% 
  mutate(agecat = cut(ind_age,
                      breaks=c(0,18,24,29,34,39,44,49,54,59,64,69,74,81),
                      labels=c("18","19-24","25-29","30-34","35-39","40-44","45-49",
                               "50-54","55-59","60-64","65-69","70-74","75-80"))) %>% 
  group_by(year, state, sex, race, agecat) %>% 
  summarise(TPop=sum(TPop))

# Generate the years from 1981 to 1989
years <- expand.grid(year = c(1981:1989,1991:1999), state=unique(wholepop$state), sex = unique(wholepop$sex),
                    race = unique(wholepop$race),
                    agecat=unique(wholepop$agecat))

# Merge the years with the data
wholepop <- merge(wholepop, years, all = TRUE)

interpolate_data <- function(data){
# find value of TPop at year 1980 and 1990 
year1980 <- data$TPop[data$year==1980]
year1990 <- data$TPop[data$year==1990]
year2000 <- data$TPop[data$year==2000]
  
diff19801990 <- year1990 - year1980
difftoadd1980_90 <- diff19801990/10
diff19902000 <- year2000 - year1990  
difftoadd1990_00 <- diff19902000/10

data <- data %>% 
  mutate(toadd = ifelse(year<1990, difftoadd1980_90, difftoadd1990_00),
         yearnum = ifelse(year<1990, year-1980, 
                          ifelse(year>=1990 & year<2000, year-1990, 0))) %>% 
  arrange(year) %>% 
  fill(TPop, .direction="down") %>% 
  mutate(TPop_interp = TPop+(toadd*yearnum))
  
return(data)
}

wholepop <- wholepop %>% group_by(state, sex, race, agecat) %>% 
  do(interpolate_data(.))


test <- wholepop %>% filter(state=="USA") %>% group_by(year, sex, agecat) %>% 
  summarise(TPop = sum(TPop_interp))
ggplot(data=test, aes(x=year, y=TPop, colour=as.factor(sex))) + geom_line() +
  facet_grid(cols=vars(agecat))

# save ACS population counts 2000 to 2021 
write.csv(wholepop, "SIMAH_workplace/ACS/ACS_popcounts_1980_2021_bystate_age_gp.csv", row.names=F)
