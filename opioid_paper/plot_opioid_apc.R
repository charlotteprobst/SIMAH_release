...
data1 <- read_dta("temp2000-20poi-age-aggregate-population.dta") %>% 
  ...
  group_by(age_gp,year,birth_gp, sex, edclass,name) %>% 
  ...
  group_by(sex, edclass, name, type, number) %>% 
  summarise(value=sum(value),
            pop=sum(pop),
            rate = (value/pop)*100000) %>% 
  mutate(edclass = recode(edclass, "1"="High school degree or less",
                          "2"="Some college",
                          "3"="College degree +"),
...
plot1 <- ggplot(men, aes(x=number, y=rate, colour=edclass)) + 
...
ggsave("Fig3_APC_plot_IRR_sex.png", plot, dpi=300, width=33, height=19, units="cm")
...
ggsave("Fig4A_APC_plot_IRR_cohort.png", cohort, dpi=300, width=33, height=19, units="cm")
