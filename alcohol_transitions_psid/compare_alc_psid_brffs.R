...
summarybrfss_complex <- brfssorig %>%  filter(gramsperday_upshifted!=0) %>% 
  group_by(YEAR, sex_recode, age_cat, education_cat, race_eth) %>% summarise(meangpd = mean(gramsperday_upshifted)) %>% 
  rename(year=YEAR, sex=sex_recode) %>% 
  mutate(sex = ifelse(sex=="Male","male","female"),
         type="BRFSS")

summarybrfss_complex <- summarybrfss_complex %>% 
  filter(!is.na(education_cat))

summary_complex <- rbind(summary_psid_complex, summarybrfss_complex) %>% filter(!is.na(age_cat))

summary_complex_M <- summary_complex %>% filter(sex=="male")
summary_complex_F <- summary_complex %>% filter(sex=="female")
