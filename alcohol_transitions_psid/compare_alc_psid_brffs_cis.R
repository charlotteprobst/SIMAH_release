...
summarybrfss_gpd <- brfssorig %>%  filter(gramsperday_upshifted!=0) %>% 
  ...
  summarise(n = n(),
            meangpd = mean(gramsperday_upshifted, na.rm = TRUE),
            sd = sd(gramsperday_upshifted, na.rm = TRUE),
            se = sd / sqrt(n),
            lower_ci = meangpd - qt(0.975, df = n - 1) * se,
            upper_ci = meangpd + qt(0.975, df = n - 1) * se) %>% 
...
summarybrfss_prevalence <- brfssorig %>% 
  group_by(YEAR, sex_recode, race_eth) %>% 
  summarise(n = n(),
            prevalence = mean(drinkingstatus, na.rm = TRUE),
            sd = sd(drinkingstatus, na.rm = TRUE),
            se = sd / sqrt(n),
            lower_ci = prevalence - qt(0.975, df = n - 1) * se,
            upper_ci = prevalence + qt(0.975, df = n - 1) * se) %>% 
  rename(year=YEAR, sex=sex_recode) %>% 
  mutate(sex = ifelse(sex=="Male","male","female"),
         type="BRFSS")
