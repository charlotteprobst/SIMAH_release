...
format_CMed <- function (model, group_label, coef_list) {
        ...
        one <- full_join(coef, prop, by="term") %>%
          mutate(label = c(paste0("02 Direct effect of race (ref=White)"),
        ...
        two <- TE %>%
          mutate (label = ifelse(term == "TE", paste0("01 Total effect of race (ref=White)"), NA),
                  prop_CI = "1") %>% 
          dplyr::select(term, label, deaths_10000py_CI, prop_CI)
        
        three <- full_join(IE, IE_prop, by="term") %>% 
          mutate (label = ifelse(term == "IE", paste0("03 Indirect effect of race (ref=White)"), NA)) %>% 
          dplyr::select(term, label, deaths_10000py_CI, prop_CI) 
        
        
        final <- rbind(one, two, three) %>%
           arrange(label) %>%
           add_row(deaths_10000py_CI = group_label, prop_CI=group_label, .before=1)
