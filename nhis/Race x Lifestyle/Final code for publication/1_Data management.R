...
nhis_all <- read_sas (paste0(data_orig, "nhis_mort_clean.sas7bdat")) %>%
  ...
  mutate (# Recode and create variables
    ...
    alcohol2 = recode(alcohol5v2, `1`=1, `2`=1, `3`=2, `4`=2, `5`=2),  # binary, 1: current non-drinker, 2: current drinker 
    
    smk = recode(smoking, `0`=1, `1`=2, `2`=3, `3`=4),                   # recoded such that '1' is the first/smallest category
    smk2 = recode(smk, `1`=1, `2`=1, `3`=2, `4`=2),  # binary, 1: current non-smoker, 2: current smoker
    
    bmi2 = recode(bmi_cat, `1`=1, `2`=1, `3`=2, `4`=2), # 1:healthy or under weight, 2:overweight or obese
    
    phy2 = recode(phy_act3, `1`=2, `2`=1, `3`=1),  # 1: active or somewhat active, 2: sedentary
    ...
    alcohol2.factor = factor(alcohol2, levels=c(1,2), labels = c("Current non-drinker", "Current drinker")),

    # Mediator 2 - BMI
    bmi.factor = factor(bmi_cat, levels=c(2,1,3,4), labels = c("Healthy weight","Underweight", "Overweight", "Obese")),
    bmi2.factor = factor(bmi2, levels=c(1,2), labels = c("Healthy or underweight", "Overweight or obese")),
    ...
    smk2.factor = factor(smk2, levels = c(1,2), labels = c("Current non-smoker", "Current smoker")),
    
    
    # Mediator 4 - Physical Activity
    phy.factor = factor(phy_act3, levels=c(3,1,2), labels = c("Active", "Sedentary", "Somewhat active")),
    phy2.factor = factor(phy2, levels=c(1,2), labels = c("Sedentary", "Active")),
