...
getTE_NotRobust <- function(CMed_model, v){
  TE <- sum(CMed_model$gamma[v]) # extract Coefficients and sum up to get the total effect
  mu <- CMed_model$gamma[v]
  Omega <- CMed_model$var.gamma[v,v]    # To obtain non-robust estimates  # covariance matrix
  temp <- mvrnorm(n=10^4, mu=mu, Sigma=Omega)  # sampling 10,000 times from a Multivariate normal distribution
  temp_TE <- apply(temp,1,sum)  # obtain the total effects for the 10,000 samples
  med_prop <- c(mu/TE,1)
...
getTE_IE_NotRobust <- function(CMed_model, v, z){
  ...
  return(output)}
      ...
      # Function to get the (Robust)  proportion mediated of the combined indirect effect
