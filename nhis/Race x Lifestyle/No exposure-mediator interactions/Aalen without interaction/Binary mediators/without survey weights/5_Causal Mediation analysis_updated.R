...
# check the interaction between race/ethnicity and alcohol
...
# Data Preparation ------------------------------------------------------------------------------------------------------------------
CMed_race3_prep(nhis18_female) %>% saveRDS(paste0(output, "expandedData_fem_unweighted.rds"))
CMed_race3_prep(nhis18_male)   %>% saveRDS(paste0(output, "expandedData_male_unweighted.rds"))


# Run Analyses, WOMEN ----------------------------------------------------------------------------------------------------------------

# Load data
expandedData <- readRDS(file.path(output, "expandedData_fem_unweighted.rds")) %>%
...
saveRDS(CMed_f, file.path(output, "CMed_f_unweighted.rds")) # Save model results      
...
CMed_model <- readRDS(file.path(output, "CMed_f_unweighted.rds"))  # load model (if needed)
...
# Load data
expandedData <- readRDS(file.path(output, "expandedData_male_unweighted.rds"))
...
saveRDS(CMed_m, file.path(output, "CMed_m_unweighted.rds"))  # Save model results     

# Load model and view results
CMed_model <- readRDS(file.path(output, "CMed_m_unweighted.rds"))  # load model (if needed)
Black <- c(1, 3, 5, 7, 9)             # List the coefficients of interest 
Hispanic <- c(2, 4, 6, 8, 10)  
format_CMed (CMed_model, Black) %>% kable()    # print formatted results
