...
data personsx1997; set lib1997.personsx;
	length PUBLICID $14; PUBLICID = trim(left(SRVY_YR||HHX||FMX||PX));	
	keep PUBLICID SEX AGE_P R_MARITL DOINGLW WHYNOWRK ORIGIN RACEREC EDUC RAT_CAT PHSTAT USBORN_P; 	
run;
...
data personsx1998; set lib1998.personsx;
	length PUBLICID $14;PUBLICID = trim(left(SRVY_YR||HHX||FMX||PX)); 						
	keep PUBLICID SEX AGE_P R_MARITL DOINGLW WHYNOWRK HISPCODE EDUC RAT_CAT PHSTAT USBORN_P;	 
run;
...
data personsx1999; set lib1999.personsx;
	length PUBLICID $14; PUBLICID = trim(left(SRVY_YR||HHX||FMX||PX));  							
	keep PUBLICID SEX AGE_P R_MARITL DOINGLW WHYNOWRK HISPCODR EDUC RAT_CAT PHSTAT USBORN_P;  
		run;
...
data personsx2000; set lib2000.personsx;
	length PUBLICID $14; PUBLICID = trim(left(SRVY_YR||HHX||FMX||PX));  							
	keep PUBLICID SEX AGE_P R_MARITL DOINGLW WHYNOWRK HISCOD_I EDUC RAT_CAT PHSTAT USBRTH_P;  
run;
...
data personsx2001; set lib2001.personsx;
	length PUBLICID $14; PUBLICID = trim(left(SRVY_YR||HHX||FMX||PX));  							
	keep PUBLICID SEX AGE_P R_MARITL DOINGLW1 WHYNOWK1 HISCOD_I EDUC RAT_CAT PHSTAT USBRTH_P;  
run;
...
data personsx2002; set lib2002.personsx;
	length PUBLICID $14; PUBLICID = trim(left(SRVY_YR||HHX||FMX||PX));  							
	keep PUBLICID SEX AGE_P R_MARITL DOINGLW1 WHYNOWK1 HISCOD_I EDUC RAT_CAT PHSTAT GEOBRTH;  
run;
...
data personsx2003; set lib2003.personsx;
	length PUBLICID $14; PUBLICID = trim(left(SRVY_YR||HHX||FMX||PX));  							
	keep PUBLICID SEX AGE_P R_MARITL DOINGLW1 WHYNOWK1 HISCODI2 EDUC RAT_CAT PHSTAT GEOBRTH;  
run;
...
data personsx2004; set lib2004.personsx;
	length PUBLICID $14; PUBLICID = trim(left(SRVY_YR||HHX||FMX||FPX));  						 
	keep PUBLICID SEX AGE_P R_MARITL HISCODI2 EDUC1 STRATUM PSU PHSTAT GEOBRTH;   /*in 2004 the PSU and STRATUM variables were only available on the person file*/ 
run;
...
data personsx2005; set lib2005.personsx;
	length PUBLICID $14; PUBLICID = trim(left(SRVY_YR||HHX||FMX||FPX));  						 
	keep PUBLICID SEX AGE_P R_MARITL HISCODI2 EDUC1 PHSTAT GEOBRTH;  
run;
...
data personsx2006; set lib2006.personsx;
	length PUBLICID $14; PUBLICID = trim(left(SRVY_YR||HHX||FMX||FPX));  						 
	keep PUBLICID SEX AGE_P R_MARITL HISCODI3 EDUC1 PHSTAT GEOBRTH;  
run;
...
/***************************************************/
data nhis_all; 
	set nhis1997 nhis1998 nhis1999 nhis2000 nhis2001 nhis2002 nhis2003 nhis2004 nhis2005 nhis2006
		nhis2007 nhis2008 nhis2009 nhis2010 nhis2011 nhis2012 nhis2013 nhis2014;
run;
...
/*ALCOHOL USE - 1997-2014*/
	/*Data is consistent from 1997-2014, except that heavy drinking has a different variable name in 2014; make data consistent*/
	if SRVY_YR = 2014 then ALC5UPYR = ALC5UPY1;
...
/*PSYCHOLOGICAL DISTRESS  - 1997-2014*/
	/*Data is consistent from 1997-2012, except that FEELINGS have a different variable name in 2013 AND 2014; make data consistent*/
...
proc freq data = nhis_all; tables effort; run;
...
data nhis_clean; set nhis_all;
	keep PUBLICID SRVY_YR INTV_QRT HHX FMX PX FPX WTFA_SA STRATUM PSU STRAT_P PSU_P 
		 alcohol_daily_grams alcohol5 alcohol6 heavy_drinker drink_hist hed
		 female age ethnicity ethnicity2 married edu employed bmi bmi_cat smoking hypertension diabet phy_act3 income US_BORN health_stat
...
data nhis_mort_clean; set nhis_mort_clean;
	...
	new_weight = sa_wgt_new / 18;  /*divided by 18 because 18 survey years were pooled form NHIS*/
	new_psu = PSU;
	if new_psu = . then new_psu = psu_p;
	new_stratum = STRATUM + 1000;
	if new_stratum = . then new_stratum = STRAT_P + 2000;
	drop WTFA_SA sa_wgt_new PSU psu_p STRATUM STRAT_P;
run;  
...
Format 	alcohol5 alc5_use. 		alcohol6 alc6_use. 		heavy_drinker heavy_drink. 	hed hed.	
		drink_hist drink_hist.		female female.			age age. 									
		ethnicity ethn.				married married.			edu edu.					
		employed employ.			bmi_cat bmi.			smoking smoke.
		hypertension hyperten.		diabet diabet.			phy_act3 phys_act.
		income income.				ethnicity2 ethn_b.		health_stat health.
		K6SCALE3 psydistress.
...
/*check*/
	proc freq data=combined.nhis_mort_clean; 
			tables alcohol5 alcohol6 heavy_drinker drink_hist HED female age ethnicity ethnicity2 married edu employed 
