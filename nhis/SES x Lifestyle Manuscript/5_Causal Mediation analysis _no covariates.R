...
causal_mediation_prep <-function(data) {
      ...
      mydata <- data %>%
        mutate(A.edu = edu,
          ...
          M3.bmi = bmi_cat,
          M4.phy = phy_act3) %>%
        dplyr::select(A.edu, M1.alc, M2.smk, M3.bmi, M4.phy, allcause_death, bl_age, end_age, srvy_yr) 
      ...
      fitM1 <- vglm(M1.alc ~ ATemp + bl_age + factor(srvy_yr), data = mydata, family=multinomial(refLevel = 3))
      fitM2 <- vglm(M2.smk ~ ATemp + bl_age + factor(srvy_yr), data = mydata, family=multinomial(refLevel = 1))
      fitM3 <- vglm(M3.bmi ~ ATemp + bl_age + factor(srvy_yr), data = mydata, family=multinomial(refLevel = 2))
      fitM4 <- vglm(M4.phy ~ ATemp + bl_age + factor(srvy_yr), data = mydata, family=multinomial(refLevel = 3))
...
# Data Preparation ----------------------------------------------------------------------------------------
causal_mediation_prep(nhis_female) %>% saveRDS(paste0(output, "expandedData_fem_noCovs.rds"))
causal_mediation_prep(nhis_male)   %>% saveRDS(paste0(output, "expandedData_male_noCovs.rds"))


# Run Analyses, WOMEN ----------------------------------------------------------------------------------------------------------------

# Load data
expandedData <-readRDS(file.path(output, "expandedData_fem_noCovs.rds"))
...
# Save model results
saveRDS(CMed_f, file.path(output, "CMed_f_noCovs.rds"))


# Load model results
CMed_model <-readRDS(file.path(output, "CMed_f_noCovs.rds"))
...
summary(CMed_model)
getTE_NotRobust(CMed_model, c(1,3,5,7,9,28,32,36,40))                             # Function to get the total effect and proportion mediated
getIE_NotRobust(CMed_model, c(3,5,7,9,28,32,36,40))                               # Function to get the total combined indirect effect
getTE_IE_NotRobust(CMed_model, c(1,3,5,7,9,28,32,36,40), c(3,5,7,9,28,32,36,40))  # Function to get the proportion mediated of the combined indirect effect
...
# Load data
expandedData <- readRDS(file.path(output, "expandedData_male_noCovs.rds"))
...
# Save model results
saveRDS(CMed_m, file.path(output, "CMed_m_noCovs.rds"))       

# Load model results
CMed_model <-readRDS(file.path(output, "CMed_m_noCovs.rds"))  
