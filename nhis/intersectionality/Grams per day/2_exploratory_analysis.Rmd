...
nhis_alc_clean %>% summarise(median(alc_daily_g), IQR(alc_daily_g)) # full survey

# Drinkers only
nhis_alc_clean %>% filter(ALCSTAT1 == "Current drinker") %>% count()
nhis_alc_clean %>% filter(ALCSTAT1 == "Current drinker") %>% group_by(age_3_cats) %>% count %>% ungroup() %>% mutate(percent=n/sum(n)*100)
nhis_alc_clean %>% filter(ALCSTAT1 == "Current drinker") %>% group_by(SEX) %>% count %>% ungroup() %>% mutate(percent=n/sum(n)*100)
nhis_alc_clean %>% filter(ALCSTAT1 == "Current drinker") %>% group_by(race_5_cats) %>% count %>% ungroup() %>% mutate(percent=n/sum(n)*100)
nhis_alc_clean %>% filter(ALCSTAT1 == "Current drinker") %>% group_by(education_3_cats) %>% count %>% ungroup() %>% mutate(percent=n/sum(n)*100)
nhis_alc_clean %>% filter(ALCSTAT1 == "Current drinker") %>% group_by(decade) %>% count %>% ungroup() %>% mutate(percent=n/sum(n)*100)
nhis_alc_clean %>% filter(ALCSTAT1 == "Current drinker") %>% summarise(median(alc_daily_g), IQR(alc_daily_g)) 

nhis_alc_clean %>% filter(ALCSTAT1 == "Current drinker") %>% 
```
