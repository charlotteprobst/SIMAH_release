...
# Calculate how many individuals would be included if we were to use person weighting
```{r}
sum(nhis_subset$SAMPWEIGHT)
# Look at whether it would be possible to divide the sample by 10

min(nhis_subset$SAMPWEIGHT)
# If divided by 10 would not lose any individuals entirely as all person weights are > 10
```
...
## Missing data by intersectional group characteristics

```{r,  warning = FALSE}
## Review missing data for full sample by combinations of key demographics:
missing <- nhis_subset_recoded %>% 
...
# boxplots of missing alc status
ggplot(missing) + geom_boxplot(aes(x = factor(age_3_cats), y = perc_missing_Alc_status))  
ggplot(missing) + geom_boxplot(aes(x = factor(race_5_cats), y = perc_missing_Alc_status))  
ggplot(missing) + geom_boxplot(aes(x = factor(education_3_cats), y = perc_missing_Alc_status)) 
ggplot(missing) + geom_boxplot(aes(x = factor(decade), y = perc_missing_Alc_status))  
ggplot(missing) + geom_boxplot(aes(x = factor(SEX), y = perc_missing_Alc_status)) 

missing_2 <- nhis_subset_recoded %>% 
  group_by(SEX, age_3_cats, race_5_cats, decade) %>%
  mutate(n=1) %>%
  summarise(total_pop = sum(n), 
            Educ_NA = sum(is.na(education_3_cats)),
            # Calculate percentages for variables with some missing data:
            perc_missing_edu = (Educ_NA/total_pop)*100) %>% 
            arrange(desc(perc_missing_edu))

# boxplots of missing education status
ggplot(missing_2) + geom_boxplot(aes(x = factor(age_3_cats), y = perc_missing_edu))  
ggplot(missing_2) + geom_boxplot(aes(x = factor(race_5_cats), y = perc_missing_edu))  
ggplot(missing_2) + geom_boxplot(aes(x = factor(decade), y = perc_missing_edu))  
ggplot(missing_2) + geom_boxplot(aes(x = factor(SEX), y = perc_missing_edu)) 
...
saveRDS(nhis_subset_dropped_alcstat_na,"C:/Users/cmp21seb/Documents/SIMAH/SIMAH_workplace/nhis/intersectionality/cleaned_data/sampleadults_droppedNA_edu_alcstat.RDS")
```

View sample size of intersectional groups now that missing data dropped
```{r}
# Generate intersections & count people in each group
...
data_intersections <- data_intersections %>%
...
sum(data_intersections$count >= 20)/nrow(data_intersections)  

intersections_reference <- subset(data_intersections, select = c(intersections, SEX, race_5_cats, education_3_cats, age_3_cats, decade)) %>% 
  unique()  %>%
  mutate(SEX = recode(SEX, "1" = "Male", "2" = "Female")) %>%
  mutate(intersectional_names = as.character(paste(SEX, age_3_cats, race_5_cats, education_3_cats, decade)))
```

Review distribution of drinkers vs non-drinkers in full sample
```{r}
# Total
...
drinking_status_1 <- merge(intersections_reference,drinking_status_1,by="intersections")

saveRDS(drinking_status,"C:/Users/cmp21seb/Documents/SIMAH/SIMAH_workplace/nhis/intersectionality/cleaned_data/sampleadults_drinkingstatus.RDS")

# By drinking status
drinking_status_2 <- data_intersections %>%
  group_by(ALCSTAT1) %>%
  count(intersections) %>%
  mutate(percent = n/sum(n)*100) %>%
  subset(select= -(n)) %>%
  pivot_wider(names_from = ALCSTAT1, values_from = percent) %>%
  round(digits=2)%>%
  rename(
    abstainers = "1",
    former_drinkers = "2",
  ...
  arrange(desc(drinkers)) 

drinking_status_2 <- merge(intersections_reference,drinking_status_2,by="intersections") %>%
    subset(select = - c(SEX,race_5_cats, education_3_cats, decade, age_3_cats))
```
...
## Remove drinkers with inconsistant alcohol data
```{r}
nhis_alc_clean <- data_with_inconsistancies %>% filter(inconsistent_alc==0)
```

## Summarise drinking status within sample adults
```{r}
sample_by_drink_stat <- nhis_alc_clean %>%
  group_by(alc_5_cats) %>%
  count() %>%
  ungroup() %>%
  mutate(percent = n/sum(n)*100)
...
# copy into excel
clipr::write_clip(sample_by_drink_stat)
```
...
#non-capped
ggplot(nhis_alc_clean, aes(x=alc_daily_g), y) + geom_histogram(bins=400) + xlim(0,201) + ylim(0,30000) + 
ggtitle("Distribution of  estimated alc_daily_g")

#capped
ggplot(nhis_alc_clean, aes(x=alc_daily_g_capped_200), y) + geom_histogram(bins=400) + xlim(0,201) + ylim(0,30000) + 
  xlab("Daily grams of alcohol") +
  ylab("Frequency") +
ggtitle("Distribution of estimated daily grams of alcohol amongst NHIS sample adults")

# Drinkers only 
# non-capped
nhis_alc_clean %>%
  filter(ALCSTAT1=="Current drinker") %>%
  ggplot(aes(x=alc_daily_g), y) + geom_histogram(bins=400) + xlim(0,201) + ylim(0,30000) + 
...
saveRDS(nhis_alc_clean, "C:/Users/cmp21seb/Documents/SIMAH/SIMAH_workplace/nhis/intersectionality/cleaned_data/4_nhis_alc_clean.RDS")

# b) Save subset of drinkers only 
nhis_alc_clean %>%
  filter(ALCSTAT1=="Current drinker") %>%
  saveRDS("C:/Users/cmp21seb/Documents/SIMAH/SIMAH_workplace/nhis/intersectionality/cleaned_data/5_nhis_alc_clean_drinkers_only.RDS")
