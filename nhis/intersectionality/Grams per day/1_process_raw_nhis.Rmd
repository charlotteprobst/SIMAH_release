...
# ddi <- read_ipums_ddi("U:/SIMAH/SIMAH_workplace/nhis/intersectionality/raw_data/nhis_00005.xml")
# data <- read_ipums_micro(ddi)

# Save extracted data as .RDS file
# saveRDS(data, "U:/SIMAH/SIMAH_workplace/nhis/intersectionality/cleaned_data/1_nhis_full_extract.RDS")
 
# Read in extracted data:
nhis_full_extract <- readRDS("U:/SIMAH/SIMAH_workplace/nhis/intersectionality/cleaned_data/1_nhis_full_extract.RDS")
```
...
nhis_subset <- readRDS("U:/SIMAH/SIMAH_workplace/nhis/intersectionality/cleaned_data/2_nhis_subset.RDS")
```
...
sum(is.na(nhis_subset_cohort$birth_year)) # 77,046 individuals missing birth year

# Generate birth year estimate manually
nhis_subset_cohort <- nhis_subset_cohort %>% 
  filter(!is.na(birth_year)) %>% 
  mutate(birth_year_est = YEAR-AGE) %>% 
# Compare accuracy of estimated versus actual birth years (for individuals with birth year known)
  mutate(difference = birth_year - birth_year_est)

nhis_subset_cohort %>% summarise(min(difference), max(difference)) # -2 to 1
...
saveRDS(sample_adults_by_sex_race_age, "U:/SIMAH/SIMAH_workplace/nhis/intersectionality/cleaned_data/sample_adults_demographics.RDS")
```
...
## Missing data by intersectiona group
This table shows the percentage of missing data for all intersectional groups.
As shown, <4% of alcohol status and education status are missing (across all combinations of age, gender, race and education).
