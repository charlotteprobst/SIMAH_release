...
Review basic stats of estimated grams per day data for full sample
```{r}
full_sample %>% summarise(min(alc_daily_g), max(alc_daily_g), mean(alc_daily_g))
full_sample %>% summarise(min(alc_daily_g_capped_200), max(alc_daily_g_capped_200), mean(alc_daily_g_capped_200))
```

Trial transformation of full sample data
```{r}
# add half of the smallest grams value (for drinkers) to zero values
full_sample <- 
  full_sample %>% mutate(new_grams = alc_daily_g_capped_200 + 0.02)
...
# qq plot
qqnorm(full_sample$capped_daily_grams_log, pch = 1, frame = FALSE)
qqline(full_sample$capped_daily_grams_log, lwd = 2)
```
