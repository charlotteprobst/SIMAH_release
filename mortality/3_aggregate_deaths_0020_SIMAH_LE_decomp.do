 ...
 *  replace drug_poi = . if opioid_poi == 1
   ...
   *}  
   ...
   ///recode alcohol poisoing 
   ///1) Underlying cause: using X45 only, F10.0 goes to AUD 
   ///2) Contributing cause assigned later to give precedence to other cuases 
   gen alc_poi_X45 = 1 if inrange(icd10, "X45", "X459") 
   
   
   /// final unintentional injury, excl MVA and all poisoning 
   gen uij = 1 if uij_temp == 1 & mvacc == . & alc_poi_X45 == . & opioid_poi == . 
   drop uij_temp
   ...
   replace othj = . if inlist(1,uij,mvacc,opioid_poi,alc_poi_X45,sij)
...
*   gen alzheimer = 1 if inrange(icd10,"G30","G309")
	...
	//Dementia: Dementia was defined by any primary or associated discharge diagnosis (ICD-10) codes labelling dementia 
	//(F00-F03, F05.1, F1x.73, or G30) or related to dementia (other degenerative diseases of the nervous system [G31], 
	//progressive vascular leukoencephalopathy [I67.3], or senility [R54])
	//Final codes to use: F00-F03, F05.1, G30, G31, R54
	//Exclude G31.2 to be included in AUD 
	//Exclude I67.3 to be included in Stroke
    gen dementia = 1 if inrange(icd10,"F0","F039") | icd10 == "F051" | inrange(icd10,"G30","G309") | ///
		inrange(icd10,"G31","G319") | /*icd10 == "I673" |*/ inrange(icd10,"R54","R549")
    replace dementia = . if icd10=="G312"
   ...
   gen kidney = 1 if inrange(icd10,"N00","N079") | inrange(icd10,"N17","N199") ///
	...
	replace liver = 1 if icd10 == "B171"

	

	// updated AUD 
	// E24.4, F10-F10.9, G62.1, G31.2, G72.1, I42.6, K29.2, K85.2, K86.0, 
	// R78.0, Y90, Y91, Y91.0 – Y91.3, Y91.9
	//Note: X45-X45.9, Y15-Y15.9 not included, different from SIMAH 
   ...
   replace oth_ncd = . if inlist(1,heart,cancer,stroke,respirat,dementia,diabetes,kidney,liver,aud)

    //clean up residual multiple coding for "others"
	foreach v of varlist mvacc opioid_poi alc_poi_X45 uij sij othj covid19 flu oth_inf {
	   replace oth_ncd = . if oth_ncd == 1 & `v' == 1
	}  
	foreach v of varlist mvacc opioid_poi alc_poi_X45 uij sij othj covid19 flu heart cancer stroke respirat dementia diabetes  ///
		kidney liver aud {
	   replace oth_inf = . if oth_inf == 1 & `v' == 1
	}   
	
   	//test each category is coded once 
	 egen test = rowtotal(mvacc opioid_poi alc_poi_X45 uij sij othj covid19 flu oth_inf heart cancer stroke respirat dementia   ///
		diabetes kidney liver aud oth_ncd)
    tab test, m	
	drop test 
	
	
    // create final alcohol poisoning just using underlying cause code X45
	// Another option is to add other contributing cause code X45, F10.0 and T51. 
	// This is not used here given complicated overlapping with other causes
*	replace alc_poi_contrb = . if inlist(1,mvacc,opioid_poi,uij,sij,othj,covid19,flu,heart,cancer,stroke, ///    
*	  respirat, dementia, diabetes, kidney, liver,aud)
	gen alc_poi = 1 if alc_poi_X45 == 1
	
   // all remaining
   gen rest = 1
   foreach v of varlist mvacc opioid_poi alc_poi uij sij othj covid19 flu oth_inf ///
      heart cancer stroke respirat dementia diabetes kidney liver aud oth_ncd {
	  	replace rest = . if `v' == 1
	  } 	       
	  	
	 //test each category is coded once 
	 egen test = rowtotal(mvacc opioid_poi alc_poi uij sij othj covid19 flu oth_inf heart cancer stroke respirat dementia   ///
		diabetes kidney liver aud oth_ncd rest)
    tab test, m
    drop test 
...
save "3_out data/1_allethn_mortbycause_0020_LE_decomp.dta", replace
...
bysort age_gp sex edclass race year: egen OPIOIDmort = total(opioid_poi)
...
bysort age_gp sex edclass race year: egen COVmort = total(covid19)
...
bysort age_gp sex edclass race year: egen DEMENTIAmort = total(dementia)
...
reshape wide Tmort MVACCmort OPIOIDmort ALCPOImort UIJmort SIJmort OTHJmort COVmort FLUmort OTHINFmort ///
	HEARTmort CANCERmort STROKEmort RESPmort DEMENTIAmort DMmort KIDNEYmort LIVERmort AUDmort OTHNCDmort ///
	RESTmort, i(year age_gp sex race) j(edclass)
sum *mort99

foreach i in T MVACC OPIOID ALCPOI UIJ SIJ OTHJ COV FLU OTHINF ///
	HEART CANCER STROKE RESP DEMENTIA DM KIDNEY LIVER AUD OTHNCD REST {
...
reshape long Tmort MVACCmort OPIOIDmort ALCPOImort UIJmort SIJmort OTHJmort COVmort FLUmort OTHINFmort ///
	HEARTmort CANCERmort STROKEmort RESPmort DEMENTIAmort DMmort KIDNEYmort LIVERmort AUDmort OTHNCDmort ///
	RESTmort, i(year age_gp sex race) j(edclass)
