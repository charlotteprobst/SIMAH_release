...
nesarc_selected$scaledweight = as.integer(nesarc_selected$weight_wave2/455)
...
m1 <-  zeroinfl(alc_rounded_2 ~ age3_2 + female.factor_2 +
                  race.factor_2 + edu3_2 + alc_rounded_1 + abstainer_1*age3_2 +
                  alc_rounded_1*medorhigh | 
...
zero_part_lp <- zero_part_coef['(Intercept)'] +
  ...
  # zero_part_coef['medorhigh'] * data_prediction$medorhigh +
  # zero_part_coef['cat2_1'] * data_prediction$cat2_1 + 
...
count_part_lp <- count_part_coef['(Intercept)'] +
  ...
  count_part_coef['medorhigh'] * data_prediction$medorhigh +
  # count_part_coef['cat2_1'] * data_prediction$cat2_1 + 
  ...
  count_part_coef['medorhigh'] * data_prediction$medorhigh + 
  count_part_coef['alc_rounded_1:medorhigh'] * data_prediction$medorhighgpd
...
abstainers <- data_prediction %>% mutate(abstainer1=ifelse(alc_rounded_1==0, 1,0),
                                         abstainer2=ifelse(predicted_values==0,1,0)) %>% 
...
data_prediction$microsim.init.alc.gpd <- data_prediction$predicted_values
data_prediction <- code_alcohol_categories(data_prediction)
...
nesarc_selected$predicted <- data_prediction$predicted_values

cor.test(nesarc_selected$predicted, nesarc_selected$alc_rounded_2)
