...
#   2.1) Average annual TP (Table 2a)   ---------------------------------------------------------------------------
    ...
    predicted_TP(model=alc4.msm, year=1, original_n = nrow(nesarc), expanded_n = nrow(nesarc_expanded)) %>% 
      write_csv(paste0(output, "Table 2a - AlcUse Annual TP.csv")) %>% 
      kable()
    
    
    # Crude (unadjusted) model
    predicted_TP(model=alc4_crude.msm, year=1, original_n = nrow(nesarc), expanded_n = nrow(nesarc_expanded)) %>% 
...
# 2.2.3) Function to extract annual TP (Supplement 1)
...
# 2.2.3) Load and set up the initial population (based on NESARC wave 1)
...
#   2.3) Simulate and compare to NESARC 3 (Table 3) ----------------------------------------------------------
...
HR_table(alc4.msm, original_n = nrow(nesarc), expanded_n = nrow(nesarc_expanded))  %>%
...
# Final, adjusted model
predicted_TP(model=hed.msm, year=1, original_n = nrow(nesarc), expanded_n = nrow(nesarc_expanded)) %>%
...
#   3.3) Compare predicted vs observed ------------------------------------------------------------------------

# 3.3.1) First, specify the covariate values
...
# 3.3.2) Function to extract and format the annual TP
...
# 3.3.3) Load and set up the initial population (based on NESARC wave 1)
...
# 3.3.4) Compare NESARC 3 Observed vs predicted () *****************************************************************************

# Predicted population: Simulate population at 11 years follow-up
...
#   3.3.5) Plot TP over multiple years (Figure S1) ******************************************************************************

# Simulate population over multiple years (over 10 years)
hed_overtime <- rbind(simulate_pop(hed_basepop, aTP_hed, transition_hed, 1),
                      simulate_pop(hed_basepop, aTP_hed, transition_hed, 2),
                      simulate_pop(hed_basepop, aTP_hed, transition_hed, 3),
                      simulate_pop(hed_basepop, aTP_hed, transition_hed, 4),
                      simulate_pop(hed_basepop, aTP_hed, transition_hed, 5),
                      simulate_pop(hed_basepop, aTP_hed, transition_hed, 6),
                      simulate_pop(hed_basepop, aTP_hed, transition_hed, 7),
                      simulate_pop(hed_basepop, aTP_hed, transition_hed, 8),
                      simulate_pop(hed_basepop, aTP_hed, transition_hed, 9),
                      simulate_pop(hed_basepop, aTP_hed, transition_hed, 10))


# Calculate proportions
hed_overtime2 <- hed_overtime %>%
  group_by (year, hed_1, predicted_cat) %>% count() %>%  ungroup() %>%
  group_by(year, hed_1) %>%
...
hed_overtime3 <- hed_overtime2 %>%
  filter(year==1) %>%
  mutate(year = 0,
    n = ifelse(hed_1 == predicted_cat, total, 0),
    pct_total = ifelse(hed_1 == predicted_cat, 100, 0))
...
rbind (hed_overtime2, hed_overtime3) %>% 
  ...
  mutate(
    ...
    predicted_cat = fct_relevel(predicted_cat, "Non-drinker", "Drinker, no HED","Occasional HED", "Monthly HED", "Weekly HED")) %>%
  ...
  scale_x_continuous(breaks=seq(0, 10, by= 2))
ggsave(paste0(output, "Figure S1b - HED over time.tiff"), dpi=600, width=12, height = 4)



# Predicted proportion at 10 years for specific transitions:

hed_overtime2 %>%
  filter(year==1 | year ==5 | year ==10) %>%
  select(year, hed_1, predicted_cat, pct_total) %>%
...
#   3.4) Hazard ratios  (Table 5) -------------------------------------------------------------------------------

# Function to extract HR results, rearrange, and correct CI to original sample size 
HR_table(hed.msm, original_n = nrow(nesarc), expanded_n = nrow(nesarc_expanded))  %>%
...
#   3.5) Transition frequencies (Tables S3b, S4b) --------------------------------------------------------------
