...
nesarc <- readRDS(paste0(data, "nesarc_all.rds")) %>%
  ...
  mutate(alc_daily_g = ifelse(alc_daily_g>200, 200, alc_daily_g), # Cap max g/day to 200
    alc5.factor_wave1 = lag(alc5.factor, 1),                         # Create 'wave 1' version of some variables
  ...
  select (idnum, wave, female, female.factor, psu, stratum, weight_wave2,  
          age, age7, alc_daily_g_wave1, alc5.factor_wave1, alc5.factor, incident_AUD, incident_AUD.factor, AUD_lifetime2, AUD_lifetime2_wave1,
          race.factor, edu3) %>%
...
mod1_all <- svyglm(incident_AUD ~ alc5.factor_wave1, family = quasibinomial, design = nesarc_srvyr)
...
# MEN and WOMEN
mod2_all <- svyglm(incident_AUD ~ alc5.factor_wave1 + age + female, family = quasibinomial, design = nesarc_srvyr)
results2_all <- tidy(mod2_all, conf.int = TRUE, exponentiate = TRUE) %>%
  mutate(group = "All", 
         adjustment = "Age, sex")

# WOMEN
mod2_women <- svyglm(incident_AUD ~ alc5.factor_wave1 + age, family = quasibinomial, design = nesarc_srvyr_female)
results2_women <- tidy(mod2_women, conf.int = TRUE, exponentiate = TRUE) %>%
  mutate(group = "Women", 
    adjustment = "Age, sex")

# MEN
mod2_men <- svyglm(incident_AUD ~ alc5.factor_wave1 + age, family = quasibinomial, design = nesarc_srvyr_male)
...
# MEN and WOMEN
mod3_all <- svyglm(incident_AUD ~ alc5.factor_wave1 + age + female + edu3 + ethnicity4, family = quasibinomial, design = nesarc_srvyr)
results3_all <- tidy(mod3_all, conf.int = TRUE, exponentiate = TRUE) %>%
  mutate(group = "All", 
    adjustment = "Age, sex, edu, ethnicity")

# WOMEN
mod3_women <- svyglm(incident_AUD ~ alc5.factor_wave1 + age + edu3 + ethnicity4, family = quasibinomial, design = nesarc_srvyr_female)
results3_women <- tidy(mod3_women, conf.int = TRUE, exponentiate = TRUE) %>%
  mutate(group = "Women", 
    adjustment = "Age, sex, edu, ethnicity")

# MEN
mod3_men <- svyglm(incident_AUD ~ alc5.factor_wave1 + age + edu3 + ethnicity4, family = quasibinomial, design = nesarc_srvyr_male)
...
rbind(results1_all, 
  ...
  mutate(across(where(is.numeric), ~ round(.x, 2)),
    ...
    OR = estimate,
    AlcoholUse = str_remove(term, "alc5.factor_wave1")) %>% 
