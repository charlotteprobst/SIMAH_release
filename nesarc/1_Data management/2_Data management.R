...
nesarc <- readRDS(paste0(data, "nesarc_raw.rds")) %>%
  ...
  mutate(
    ...
    # TODO: Check the number used for ounce conversion. This may be wrong
...
nesarc <- nesarc %>%
  ...
  select(idnum, wave, wave.factor, psu, stratum, weight, weight_wave2, 
         ...
         alc_daily_g, alc6, alc6.factor, alc5, alc5.factor, alc5_v2, alc5_v2.factor, alc4, alc4.factor, hed, hed.factor, 
         AUD_lifetime, AUD_lifetime2,
         coolers_daily_g, beers_daily_g, wine_daily_g, liquor_daily_g, coolers_prop, beers_prop, wine_prop, liquor_prop, 
         female_w1, race_w1) %>% 
  ...
  group_by(idnum) %>%
    mutate(lost = ifelse(n()>1, 0, 1),                   # Identify those with data at one time point (8,440 observations removed; n=69,306)
           lost = ifelse(any(is.na(alc5)), 1, lost),     # Identify those with missing alcohol data in either time point (502 observations removed; n=68,804)
           lost = ifelse(any(is.na(hed)), 1, lost),      # Identify those with missing HED in either time point (108 observations removed; n=68,696)
...
nesarc3 <- nesarc3 %>%
  ...
  # identify those with missing data; 177 observations with missing data
  mutate (lost = ifelse(is.na(alc5), 1, 0),    
          lost = ifelse(is.na(hed), 1, lost)) 
