...
le_graph <- ggplot(data = dle_results, aes(x = Year, y = Life_expectancy, colour = SES)) + 
   facet_grid(rows = vars(Sex), cols = vars(SES)) +
...
le_graph
ggsave(paste0("~/Desktop/CAMH/Fall 2023/SIMAH II/Output/LE_", k.run, k.pop_type, ".jpg"), 
...
## Display results for weights - we do not have weights, code wass not run beyond this point
