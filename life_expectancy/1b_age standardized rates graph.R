...
dDescStat <- dDescStat %>% mutate(sumrow= LVDCmort + PANCmort +  DMmort + 
                                     IHDmort + HYPHDmort + HSTRmort +
                                     AUDmort + UIJmort + MVACCmort + CANmort +
                                     LRImort + IJmort) %>% 
...
dMort <- dMort %>% select(all_of(sel.vars))
...
summary <- df %>% group_by(year, sex, edclass) %>% 
      summarise(T_as = sum(Trate_as) *100000, LVDC_as = sum(mx_LVDCrate_as)*100000, 
                PANC_as = sum(mx_PANCrate_as)*100000, DM_as = sum(mx_DMrate_as)*100000, 
                HSTR_as = sum(mx_HSTRrate_as)*100000, IHD_as = sum(mx_IHDrate_as)*100000,
                HYPHD_as = sum(mx_HYPHDrate_as)*100000, 
                AUD_as = sum(mx_AUDrate_as)*100000, UIJ_as = sum(mx_UIJrate_as)*100000, MVACC_as = sum(mx_MVACCrate_as)*100000, 
                IJ_as = sum(mx_IJrate_as)*100000, 
                CAN_as = sum(mx_CANrate_as)*100000, LRI_as = sum(mx_LRIrate_as)*100000,
                REST_as = sum(mx_RESTrate_as)*100000)


summary <-  summary %>% select(year, sex, edclass, LVDC_as, PANC_as, DM_as, 
                               IHD_as, HSTR_as, 
                               HYPHD_as, AUD_as, UIJ_as, 
                               MVACC_as, IJ_as, CAN_as, LRI_as)

data_graph <- gather(data = summary, key = "cause", value = "rate", 
                         -sex , -edclass, -year)

levels(data_graph$edclass) <- list("High education" = "College", 
                                   "Middle education" = "SomeC", "Low education" = "LEHS")
...
levels(data_graph$cause) <- list("Alcohol use disorder" = "AUD_as", 
                                              ...
                                              "Pancreatitis" = "PANC_as",
                                              "Suicide" = "IJ_as",
                                              "Motor vehicle accident" = "MVACC_as", 
                                              "Unintentional injury*" = "UIJ_as",   
                                              "IHD & ischemic stroke" = "IHD_as", 
                                              "Hemorrhagic Stroke" = "HSTR_as",
...
ggplot(data=data_graph[data_graph$sex == "Men",], aes(x=year, y=rate, colour=edclass)) + 
      ...
      labs(color="SES")
ggsave("SIMAH_workplace/life_expectancy/3_graphs/1b_as mortality rates by cause and sex_men.jpeg", dpi=600, width=20, height=29, units="cm")
...
ggplot(data=data_graph[data_graph$sex == "Women",], aes(x=year, y=rate, colour=edclass)) + 
   ...
   scale_color_manual(values = color.vec) +
   labs(color="SES")
ggsave("SIMAH_workplace/life_expectancy/3_graphs/1b_as mortality rates by cause and sex_women.jpeg", dpi=600, width=20, height=29, units="cm")
