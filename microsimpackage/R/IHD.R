...
IHD <- function(data,lhs){
  ...
  data <- data %>%
    mutate(RR_IHD = ifelse(alc_gpd<1.3 & sex=="m",
                           exp(B_IHD1_MEN),
                           ifelse(alc_gpd<25 & sex=="m",
                                  exp(B_IHD2_MEN),
                                  ifelse(alc_gpd<45 & sex=="m",
                                         exp(B_IHD3_MEN),
                                         ifelse(alc_gpd<65 & sex=="m",
                                                exp(B_IHD4_MEN),
                                                ifelse(alc_gpd>=65 & sex=="m",
                                                       exp(B_IHD5_MEN),
                                                       ifelse(alc_gpd<1.3 & sex=="f",
                                                              exp(B_IHD1_WOMEN),
                                                              ifelse(alc_gpd<25 & sex=="f",
                                                                     exp(B_IHD2_WOMEN),
                                                                     ifelse(alc_gpd<45 & sex=="f",
                                                                            exp(B_IHD3_WOMEN),
                                                                            ifelse(alc_gpd<65 & sex=="f",
                                                                                   exp(B_IHD4_WOMEN),
                                                                                   ifelse(alc_gpd>=65 & sex=="f",
                                                                                          exp(B_IHD5_WOMEN), NA)))))))))),
           RR_IHD = ifelse(formerdrinker==1 & sex=="m",
  ...
  #   data <- data %>%
  #   mutate(RR_IHD = ifelse(alc_gpd<= 1.3, exp(B_IHD1),
  #                                  ifelse(alc_gpd<=24.99, exp(B_IHD2),
  #                                         ifelse(alc_gpd<=44.99, exp(B_IHD3),
  #                                                ifelse(alc_gpd<=64.99, exp(B_IHD4), exp(B_IHD5) )))),
