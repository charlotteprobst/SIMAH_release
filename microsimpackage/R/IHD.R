...
IHD <- function(data,lhs){
  ...
  data <- data %>%
    mutate(RR_IHD = ifelse(microsim.init.alc.gpd<1.3 & microsim.init.sex=="m", 
                           exp(B_IHD1_MEN), 
                           ifelse(microsim.init.alc.gpd<25 & microsim.init.sex=="m", 
                                  exp(B_IHD2_MEN), 
                                  ifelse(microsim.init.alc.gpd<45 & microsim.init.sex=="m",
                                         exp(B_IHD3_MEN), 
                                         ifelse(microsim.init.alc.gpd<65 & microsim.init.sex=="m", 
                                                ...
                                                ifelse(microsim.init.alc.gpd>=65 & microsim.init.sex=="m",
                                                       ...
                                                       ifelse(microsim.init.alc.gpd<1.3 & microsim.init.sex=="f", 
                                                              exp(B_IHD1_WOMEN), 
                                                              ifelse(microsim.init.alc.gpd<25 & microsim.init.sex=="f", 
                                                                     exp(B_IHD2_WOMEN), 
                                                                     ifelse(microsim.init.alc.gpd<45 & microsim.init.sex=="f",
                                                                            exp(B_IHD3_WOMEN), 
