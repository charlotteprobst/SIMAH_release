...
transition_alcohol_regression <- function(data, model){
 # create a dataset with the required variables dummy coded
  ...
  zero_part_coef <- model %>% filter(...2=="zero")
  ...
  zero_part_lp <- as.numeric(zero_part_coef['(Intercept)']) +
    ...
    as.numeric(zero_part_coef['alc5.factor_1Former']) * data_prediction$formerdrinker +
    as.numeric(zero_part_coef['alc5.factor_1Category I']) * data_prediction$cat1 +
    as.numeric(zero_part_coef['alc5.factor_1Category II']) * data_prediction$cat2 +
    as.numeric(zero_part_coef['alc5.factor_1Category III']) * data_prediction$cat3

  # Compute the probability of zero using the logistic function
  zero_prob <- 1 / (1+exp(-zero_part_lp))

  count_part_coef <- model %>% filter(...2=="count")
  ...
  count_part_lp <- as.numeric(count_part_coef['(Intercept)']) +
    ...
    as.numeric(count_part_coef['alc5.factor_1Former']) * data_prediction$formerdrinker +
    as.numeric(count_part_coef['alc5.factor_1Category I']) * data_prediction$cat1 +
    as.numeric(count_part_coef['alc5.factor_1Category II']) * data_prediction$cat2 +
    as.numeric(count_part_coef['alc5.factor_1Category III']) * data_prediction$cat3
