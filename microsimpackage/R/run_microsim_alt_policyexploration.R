...
run_microsim_alt <- function(seed,samplenum,basepop,brfss,
                         ...
                         policy=0, policy_int, policymodel, year_policy, scenario, 
...
if(output=="percentreduction" & y==year_policy){
  Policy[[paste(y)]] <- percentreduction %>% 
    mutate(seed=seed,
  ...
  if(y == year_policy-1) {
    ...
    alccat <- basepop %>% 
      mutate(agecat=cut(age,
                        breaks=c(0,24,64,100),
                        labels=c("18-24","25-64","65+"))) %>%
      mutate(!!paste0("alc_cat_", year_policy-1) := alc_cat) %>% 
      dplyr::select(ID, !!paste0("alc_cat_", year_policy-1))
...
}else if(output=="percentreduction"){
  Summary <- do.call(rbind, Policy)
}
