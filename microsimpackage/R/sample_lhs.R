...
sample_lhs <- function(N_SAMPLES, PE, DISEASES=diseases){
  # setup the list of priors for all possible disease outcomes
    prior <- list(
      LVDC <- list(
        ...
        c("qunif", 0.03655479, 0.03655479), #MBLIVER1
        c("qunif", -0.00011081, -0.00011081), #MBLIVER2
        c("qunif", 0.06064636, 0.06064636), #FBLIVER1
        c("qunif", -0.00031181, -0.00031181), #FBLIVER2
        c("qunif", 0.8297913, 0.8297913), #LIVER FORMER DRINKERS
        c("qunif", -0.01, 0.05)), #BASE RATES FACTOR
  ...
  names(prior$LVDC) <- c("B_LIVER1_MEN","B_LIVER2_MEN",
                    "B_LIVER1_WOMEN","B_LIVER2_WOMEN", "LIVER_FORMERDRINKER","BASERATES_FACTOR")
  ...
  for(i in 1:nrow(lhsSample)){
    list[[paste(i)]] <- lhsSample %>% filter(SampleNum==i)
  }
  lhsSample <- list
  }
