...
CirrhosisAll <- function(data,lhs){
  ...
  data <- data %>%
    mutate(RR_LVDC = ifelse(alc_gpd<183.75 & sex=="m",
                       exp(0 + B_LIVER1_MEN*alc_gpd + B_LIVER2_MEN*(alc_gpd^2)),
                       ifelse(alc_gpd>=183.75 & sex=="m",
                              exp(0 + B_LIVER1_MEN*183.75 + B_LIVER2_MEN*(183.75^2)),
                              ifelse(alc_gpd<97.2 & sex=="f",
                                     exp(0 + B_LIVER1_WOMEN*alc_gpd + B_LIVER2_WOMEN*(alc_gpd^2)),
                                     ifelse(alc_gpd>=97.2 & sex=="f",
