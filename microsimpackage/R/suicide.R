...
SUICIDE <- function(data,lhs){
  ...
  data <- data %>%
    mutate(RR_SUICIDE = ifelse(microsim.init.alc.gpd<50 & microsim.init.sex=="m",
           ...
           RR_SUICIDE = ifelse(formerdrinker==1 & microsim.init.sex=="m",
                               exp(LIVER_FORMERDRINKER_MEN),
                               ifelse(formerdrinker==1 & microsim.init.sex=="f",
                                      exp(LIVER_FORMERDRINKER_WOMEN), RR_SUICIDE)))
  return(data)
}
