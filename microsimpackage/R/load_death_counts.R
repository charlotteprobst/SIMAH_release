#' Loads death count data for SIMAH or CASCADE versions
#'
#' @param
#' @keywords death counts
#' @export
#' @examples
#' load_death_counts
...
deathcounts <- read.csv(paste0(DataDirectory,"allethn_sumCOD_0022_SIMAH.csv")) %>%
...
}else{deathcounts <- read.csv(paste0(WorkingDirectory,"sumCOD_0018_mortcount_state.csv")) %>% filter(age_gp!=80) %>% mutate(Sex=recode(sex, "1"="m","2"="f"),
       agecat = recode(age_gp, "18"="18-24","25"="25-29",
                       "30"="30-34","35"="35-39","40"="40-44",
                       "45"="45-49","50"="50-54","55"="55-59",
                       "60"="60-64","65"="65-69","70"="70-74","75"="75-79","80"="80+"),
  ...
  names(deathcounts)[3:12] <- gsub("U_", "", names(deathcounts)[3:12])
  deathcounts <- deathcounts %>% mutate_at(vars(LVDCmort, DMmort,
                   IHDmort, ISTRmort, HYPHDmort, AUDmort, UIJmort, MVACCmort, IJmort, RESTmort), fun)
