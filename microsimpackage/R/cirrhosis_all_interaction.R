...
CirrhosisAllInteraction <- function(data,lhs){
  ...
  # FORMERDRINKER <- as.numeric(lhs["LIVER_FORMERDRINKER"])
    ...
    data <- data %>%
    mutate(RR_LVDC = ifelse(education=="LEHS",
                                exp(B_LIVER_LEHS + B_LIVER_GPD*(alc_gpd/20) + B_LIVER_LEHSxGPD*(alc_gpd/20)),
                                ifelse(education=="SomeC",
                                      exp(B_LIVER_SomeC + B_LIVER_GPD*(alc_gpd/20) + B_LIVER_SomeCxGPD*(alc_gpd/20)),
                                      exp(B_LIVER_GPD*(alc_gpd/20)))),
           RR_LVDC = ifelse(formerdrinker==1 & education=="LEHS",
...
#  mutate(RR_LVDC = ifelse(education=="LEHS" & sex=="m",
#                              exp(B_LIVER_LEHS_MEN + B_LIVER_GPD_MEN*alc_gpd + B_LIVER_LEHSxGPD_MEN*alc_gpd),
#                              ifelse(education=="SomeC" & sex=="m",
#                                    exp(B_LIVER_SomeC_MEN + B_LIVER_GPD_MEN*alc_gpd + B_LIVER_SomeCxGPD_MEN*alc_gpd),
#                                    ifelse(education=="College" & sex=="m",
#                                           exp(B_LIVER_GPD_MEN*alc_gpd),
#                                           ifelse(education=="LEHS" & sex=="f",
#                                                  exp(B_LIVER_LEHS_WOMEN + B_LIVER_GPD_WOMEN*alc_gpd + B_LIVER_LEHSxGPD_WOMEN*alc_gpd),
#                                                  ifelse(education=="SomeC" & sex=="f",
#                                                        exp(B_LIVER_SomeC_WOMEN + B_LIVER_GPD_WOMEN*alc_gpd + B_LIVER_SomeCxGPD_WOMEN*alc_gpd),
#                                                        ifelse(education=="College" & sex=="f",
#                                                               exp(B_LIVER_GPD_WOMEN*alc_gpd), NA)))))),
