...
inward_births_rate <- function(basepop, migration_rates, y, brfss){
  ...
  denominator <- basepop %>%
    ...
    group_by(race, sex) %>%
    tally()
  births <- left_join(births, denominator, by=c("race","sex"))
  births$toadd <- births$n*births$birthrate
#
#   summary <- migration_counts %>% filter(Year==y) %>%
#     mutate(toadd = MigrationInN*proportion) %>%
#     dplyr::select(agecat, race, sex, toadd)
