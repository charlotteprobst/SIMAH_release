...
for (disease in diseases) {
  ...
  summary_list[[paste0(disease)]] <- Output %>%
    ...
    left_join(.,age2010) %>%
    summarise(!!paste0("observed_", disease) := round(sum(!!sym(paste0(disease)))),
              !!paste0("simulated_",disease) := round(sum(!!sym(paste0("mort_", disease)))),
...
plot1 <- ggplot(data=subset(summary, sex=="Women"), aes(x=year, y=value, colour=type)) +
...
plot2 <- ggplot(data=subset(summary, sex=="Men"), aes(x=year, y=value, colour=type)) +
  geom_line(linewidth=2) +
  facet_grid(cols=vars(education),
             rows=vars(cause), scales="free") +
  ylab("Age-st Mortality per 100,000") +
