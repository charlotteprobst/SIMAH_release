...
#' @export
#' @examples
#' transition_alcohol_multinom_regression
transition_alcohol_multinom_regression <- function(data, model,y){
 # create a dataset with the required variables dummy coded
  ...
  predictors[[paste(i)]] <- as.numeric(coefs['X.Intercept.']) +
  ...
  predictor_nd <- predictors[["Non-drinker"]]
  predictor_lr <- predictors[["Low risk"]]
  predictor_mr <- predictors[["Medium risk"]]
  predictor_hr <- predictors[["High risk"]]

  sumpredictors <- predictor_nd + predictor_lr + predictor_mr + predictor_hr

  data_prediction$prob_nondrinker <- predictor_nd / sumpredictors
  data_prediction$prob_lowrisk <- predictor_lr / sumpredictors
  data_prediction$prob_mediumrisk <- predictor_mr / sumpredictors
  data_prediction$prob_highrisk <- predictor_hr / sumpredictors

  data_prediction$cprob_nondrinker <- data_prediction$prob_nondrinker
  data_prediction$cprob_lowrisk <- data_prediction$cprob_nondrinker + data_prediction$prob_lowrisk
  data_prediction$cprob_mediumrisk <- data_prediction$cprob_lowrisk + data_prediction$prob_mediumrisk
  data_prediction$cprob_highrisk <- data_prediction$cprob_mediumrisk + data_prediction$prob_highrisk
