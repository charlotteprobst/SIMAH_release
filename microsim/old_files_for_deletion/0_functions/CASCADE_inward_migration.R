...
migin <- year_rates %>% dplyr::filter(Year==y) %>% dplyr::select(agecat, microsim.init.sex,
...
pool <- brfss %>% dplyr::filter(State==SelectedState) %>% dplyr::filter(YEAR>=windowmin & YEAR<=windowmax) %>% 
...
if(length(missing)>0){
  ...
  supp <- brfss %>% dplyr::filter(region==unique(pool$region)) %>% 
    dplyr::filter(YEAR>=windowmin & 
                                                                     YEAR<=windowmax) %>% 
...
toadd <- left_join(pool, tojoin) %>% dplyr::filter(toadd!=0) %>% group_by(cat) %>% 
  mutate(toadd = round(toadd, digits=0)) %>% do(sample_n(.,unique(toadd), replace=T)) %>% 
...
toadd$microsim.init.spawn.year <- NULL
# toadd$agecat <- NULL
basepop <- rbind(basepop, toadd)

list <- list(basepop,toadd)
