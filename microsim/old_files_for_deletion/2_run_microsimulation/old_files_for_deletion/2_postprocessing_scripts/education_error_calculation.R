...
summary <- df %>% group_by(microsim.init.sex, edclass, datatype, year) %>% 
  ...
  mutate(differenceACS = abs(microsim-ACS)*100,
         ...
         differencePSID = abs(microsim-PSID)*100) %>% ungroup() %>% 
  ...
  pivot_wider(names_from=microsim.init.sex, values_from=c(Mean,SD)) %>% 
...
summary <- df %>% group_by(microsim.init.sex, edclass, datatype, year) %>% 
  ...
  group_by(microsim.init.sex, edclass) %>% 
  summarise(meanACS=round(mean(differenceACS, na.rm=T),digits=2),
            sdACS = round(sd(differenceACS, na.rm=T),digits=2),
            meanCensus=round(mean(differenceCensus, na.rm=T),digits=2),
            sdCensus=round(sd(differenceCensus, na.rm=T), digits=2),
...
write.csv(summary, "3_output_data/SuppTable2p1.csv", row.names=F)
...
write.csv(summary, "3_output_data/SuppTable2p2.csv", row.names=F)
