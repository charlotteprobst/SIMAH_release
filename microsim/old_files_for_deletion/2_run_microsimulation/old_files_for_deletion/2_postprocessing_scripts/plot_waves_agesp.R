...
files <- (Sys.glob(paste("SIMAH_workplace/microsim/2_output_data/calibration_output_decay_agesp/Cirrhosis*.RDS", sep="")))
index <- c(1,10,11,12,13,14,15,2,3,4,5,6,7,8,9)
files
files <- files[order(index)]
...
ggplot(data=subset(subset, sex=="f"), aes(x=year, y=target)) + 
  ...
  ylab("Mortality rate per 100,000 population") + ylim(0,NA)
ggsave(paste0("SIMAH_workplace/microsim/2_output_data/publication/SuppFig3.png"),
...
ggplot(data=subset(subset, sex=="m"), aes(x=year, y=target)) + 
  ...
  ylab("Mortality rate per 100,000 population") + ylim(0,NA)
ggsave(paste0("SIMAH_workplace/microsim/2_output_data/publication/SuppFig4.png"),
       dpi=300, width=32, height=21, units="cm")
