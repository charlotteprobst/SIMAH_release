...
mort <- read.csv("SIMAH_workplace/microsim/1_input_data/allethn_sumCOD_0022_SIMAH.csv") %>% 
  ...
  dplyr::select(Year, agecat, microsim.init.sex, EDUC, microsim.init.race, RESTmort)
...
mort <- mort %>% 
  ...
  group_by(Year, agecat, microsim.init.sex, EDUC, microsim.init.race) %>% 
...
mort$rate <- (mort$RESTmort / mort$TotalPop) * 100000
