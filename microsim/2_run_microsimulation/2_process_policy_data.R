...
Output <- read_rds("SIMAH_workplace/microsim/2_output_data/policy_experiments/Output_policyexperiments-1.RDS")
...
for (disease in diseases) {
  ...
  summary_list[[paste0(disease)]] <- Output %>%
    ungroup() %>%
    group_by(samplenum, year, agecat, sex, education) %>%
    # left_join(.,age2010) %>%
...
summary <- Output %>% dplyr::select(samplenum,year,agecat, sex,education) %>%
  distinct()

# now join together to make a diseases dataframe for that year
for(disease in diseases){
  summary <-
    left_join(summary, summary_list[[paste0(disease)]], by=c("samplenum","year","sex","agecat","education"))
}
...
summary_agest <- summary %>% 
  group_by(samplenum, percentreduction, cause, year, sex, agecat, education) %>% 
  ...
  ungroup() %>% 
  group_by(samplenum, percentreduction, cause, year, sex, education) %>% 
...
write.csv(summary_agest, "SIMAH_workplace/microsim/2_output_data/policy_experiments/processed_calibration_data.csv",
          row.names=F)
