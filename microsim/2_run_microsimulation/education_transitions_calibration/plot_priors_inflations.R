...
data <- read_csv(paste0(DataDirectory, "/prior_range_inflations.csv")) %>% 
  ...
  group_by(YEAR, samplenum,inflation, SEX, AGECAT, RACE,
           EDUC) %>% 
  summarise(n=sum(n)) %>% 
  ungroup() %>% 
  group_by(YEAR, samplenum, inflation, SEX, AGECAT, RACE) %>% 
...
summary <- data %>% 
  group_by(YEAR, inflation, SEX, AGECAT, RACE, EDUC) %>% 
  summarise(min=min(prop),
            max=max(prop),
            target=mean(target),
            se=mean(SE))

ggplot(data=subset(summary, SEX=="Women" & AGECAT=="18-24" & EDUC=="LEHS"), 
       aes(x=as.numeric(YEAR), fill=as.factor(inflation))) + 
