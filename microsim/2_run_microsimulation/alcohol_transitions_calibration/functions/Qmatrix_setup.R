setupQ <- function(estimates){
  ...
  for(i in seq){
    template1 <- template
    template1[1,3] <- estimates[i]
    template1[2,3] <- estimates[i+1]
    template1[3,2] <- estimates[i+2]
    template1[3,4] <- estimates[i+3]
    template1[4,3] <- estimates[i+4]
    template1[4,5] <- estimates[i+5]
    template1[5,4] <- estimates[i+6]
    Qmatrices[[paste(i)]] <- template1
  }
  Qmatrices[[10]] <- Qmatrices[[1]]
  Qmatrices[[1]][1,3] <- log(abs(Qmatrices[[1]][1,3]))
