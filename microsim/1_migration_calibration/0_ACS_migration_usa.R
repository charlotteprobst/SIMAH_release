...
births$MigrationInN_impute <- ifelse(births$Year>=2001 & births$Year<=2005, NA, 
                                     ifelse(births$Year>=2020 & births$Year<=2021, NA,
                                            births$MigrationInN))
...
migrants <- rbind(migrants,missingyear)

write.csv(migrants, "SIMAH_workplace/microsim/population_data/migration_in_USA.csv", row.names=F)
