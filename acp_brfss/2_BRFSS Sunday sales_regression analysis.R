...
pdat <- data %>%
  ...
  mutate(age_gr = as.factor(ifelse(age_var < 35, "18-34", ifelse(age_var >= 35 & age_var < 50, "35-49", 
         ...
         z.unemp.rate = (log(unemp.rate) - mean(log(unemp.rate))) / sd(log(unemp.rate))) # across the whole sample
...
ggsave(paste0("acp_brfss/outputs/figures/", DATE, "_BRFSSS_INTERACT.jpg"), dpi = 300, width = 12, height = 8)
...
out.alccat.m <- broom.mixed:x:tidy(alccat.m, conf.int = TRUE, exponentiate = TRUE, effects = "fixed") %>% 
  select(term, estimate, conf.low, conf.high, p.value)
