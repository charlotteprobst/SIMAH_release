...
pdat <- DATA %>% group_by(edclass, sex) %>%
  ...
  mutate(EDCLASS = factor(ifelse(EDCLASS == "LEHS", "low", ifelse(EDCLASS == "SomeC", "medium", ifelse(EDCLASS == "College", "high", NA))),
                          levels = c("low", "medium", "high")))

fig <- ggplot(data = pdat, aes(y = change, x = QYEAR, colour = condition)) + 
...
ggsave(paste0("acp_brfss/outputs/figures/", DATE, "_TS_0019_CONDITIONchange.jpg"), fig, dpi = 300, width = 14, height = 8)
