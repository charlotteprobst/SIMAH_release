...
##### adapt the function below to include covariates for time period (for models 5 and 6)
# 
# extractTP_detailed <- function(model,combo){
#   probs <- list()
#   for(i in 1:nrow(combo)){
#     agecat <- combo$agecat[i]
#     sex <- combo$sex[i]
...
# modelt5_TPs_detail <- extractTP(modelt5, combo)
# modelt6_TPs_detail <- extractTP(modelt6, combo)
