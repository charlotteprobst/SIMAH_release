...
write.csv(data, "SIMAH_workplace/reweighted_PSID_2019_IDs.csv", row.names=F)

#### SCRIPT CAN BE STARTED FROM HERE IF REWEIGHTED DATA WITH IDS EXISTS ####
data <- read_csv("SIMAH_workplace/reweighted_PSID_2019_IDs.csv")
...
educ.msm1 <- msm(educNUM~year, newID, data=data1, qmatrix=Q,
                 center=FALSE,
                 covariates=~agescaled+agesqscaled+sex+racefinal,
