...
tojoin <- complete %>% dplyr::select(.imp, uniqueID, mother_num2, father_num2, racefinal) %>% 
  rename(imp = .imp, racefinal2 = racefinal)

education <- education %>% dplyr::select(uniqueID, year, relationshiptohead, sex, age, highestEd,
                                         weight, racefinal, father_num, mother_num)

educationList <- list()
for(i in 1:20){
  data <- subset(tojoin, imp==i)
  educationList[[paste(i)]] <- left_join(education, data)
}

education <- do.call(rbind, educationList)
...
education <- education %>% 
  mutate(mother_num = as.numeric(as.character(mother_num2)),
         father_num = as.numeric(as.character(father_num2)),
    parental_education_sum = sum(mother_num, father_num, na.rm=T),
  ...
  dplyr::select(-c(mother_num2, father_num2))
...
neweducation <- neweducation %>% 
  dplyr::select(uniqueID, imp, year, relationshiptohead, sex, age, 
                highestEd, racefinal, racefinal2, 
                father_num, mother_num, bothLEHS,
                oneCollegeplus, bothCollegeplus, maxBoth)

saveRDS(neweducation, "SIMAH_workplace/education_transitions/PSID_reweighted_2019_imputed_parental_income.RDS")                                                                                        
