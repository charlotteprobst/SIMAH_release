...
df <- read.csv("output_data/microsim_deaths_summary_race.csv")

# group by only race (not split by sex)
df <- df %>% group_by(year, raceeth, edclass, agecat, cause, datatype) %>% summarise(totaldeaths=sum(totaldeaths),
                                                                                     n=sum(n))
...
age2018 <- age2018 %>% group_by(edclass, raceeth, agecat, datatype) %>% #totals by year,sex, edclass, agecat and datatype
  summarise(n=sum(n)) %>% ungroup() %>% group_by(raceeth, edclass, datatype) %>% #group by everything except age to get proportions
...
write.csv(summary, "output_data/1_mortality_rates_data_race.csv")
