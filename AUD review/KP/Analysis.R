...
lin_mod <- dosresmeta(formula=log_RR ~ alc_daily_g, proc="1stage", 
    ...
    summary(lin_mod)
    predict(lin_mod, delta=14, exp=TRUE) # delta specifies dose increase of interest; e.g., delta=1 will give the RR associated with a 1-unit increase
...
# Cubic trend - One stage dose-response meta-analysis 
cube_mod <- dosresmeta(formula=log_RR ~ alc_daily_g + I(alc_daily_g^2) + I(alc_daily_g^3) , proc="1stage", 
  id=id_study, type="cc", se=se, cases=outcome_n, n=total_n, 
  data=data_all, control = list(maxiter = 10000))
...
# Plot
predict(cube_mod, data.frame(alc_daily_g=seq(0, 200, 1)), order=TRUE, exp=TRUE) %>% 
  ggplot(aes(x=alc_daily_g, y=pred)) + geom_line() + 
  geom_ribbon(aes(ymin= ci.lb, ymax=ci.ub), alpha=0.1) + 
  coord_cartesian(ylim=c(-5, 50))+
  theme_bw()
